...
export default function BoardReferencePage() {
  ...
  const components = (selected?.components ?? []).filter((component) => !component.cls.includes('fiducial'))
  const fiducials = (selected?.components ?? []).filter((component) => component.cls.includes('fiducial'))
